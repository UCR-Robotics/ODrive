...
void ASCII_protocol_process_line(const uint8_t* buffer, size_t len, StreamSink& response_channel) {
    ...
    /* From Gustavo */
        ...
        if (result != 1) {
            ...
            Axis* axis0 = axes[0];
            Axis* axis1 = axes[1];
            axis0->controller_.set_current_setpoint(motor0_cur_sp);
            axis1->controller_.set_current_setpoint(motor1_cur_sp);
            axis0->watchdog_feed();
            axis1->watchdog_feed();
            /* From Gustavo */
    ...
    /* From Gustavo */
        ...
        if (result != 1) {
            ...
            // axes[0]->controller_.set_coupled_setpoints(theta_sp, gamma_sp); // Original odrive code
            // axes[1]->controller_.set_coupled_setpoints(theta_sp, gamma_sp); // Original odrive code

            /* From Gustavo */
            Axis* axis0 = axes[0];
            Axis* axis1 = axes[1];
            axis0->controller_.set_coupled_setpoints(theta_sp, gamma_sp);
            axis1->controller_.set_coupled_setpoints(theta_sp, gamma_sp);
            /* From Gustavo */
    ...
    /* From Gustavo */
        ...
        if (result != 1) {
            ...
            axis0->controller_.set_coupled_setpoints(sp_theta, sp_gamma);
            axis0->controller_.set_coupled_gains(kp_theta, kd_theta, kp_gamma, kd_gamma);

            axis1->controller_.set_coupled_setpoints(sp_theta, sp_gamma);
            axis1->controller_.set_coupled_gains(kp_theta, kd_theta, kp_gamma, kd_gamma);
            /* From Gustavo */
...
/* Most of the following function is similar to teensy state machine */
/**
...
void ASCII_protocol_parse_stream(const uint8_t* buffer, size_t len, StreamSink& response_channel) {
    ...
    while (len--) {
        /* From Gustavo: added from ODrive */
...
/* From Gustavo: Original function in ODrive */
void ASCII_protocol_parse_stream_original(const uint8_t* buffer, size_t len, StreamSink& response_channel) {
    static uint8_t parse_buffer[MAX_LINE_LENGTH];
    static bool read_active = true;
    static uint32_t parse_buffer_idx = 0;
